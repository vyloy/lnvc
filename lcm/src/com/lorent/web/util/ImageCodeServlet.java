...
public class ImageCodeServlet extends HttpServlet {
	...
	private String fontFimaly = "Arial";
	/**
	 * 
	 */
...
//		获取图片上下文
		...
		graphics.setFont(new Font(fontFimaly,Font.PLAIN,fontSize));
...
//		随机产生4位验证码
		for(int i=0;i<4;i++){
			...
			graphics.drawString(rand, 8*i+2, 14);
		}
//		将验证码保存在session中
		session.setAttribute(PropertiesUtil.getProperty("Const", "session.name.imageCode"), code);
		graphics.dispose();
//		向页面输出图片
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if(config.getInitParameter("fontSize")!=null)
			fontSize = Integer.parseInt(config.getInitParameter("fontSize"));
		if(config.getInitParameter("fontFimaly")!=null)
			fontFimaly = config.getInitParameter("fontFimaly");
	}
