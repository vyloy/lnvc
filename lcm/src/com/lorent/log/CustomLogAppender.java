...
public class CustomLogAppender extends FileAppender {
    ...
    void rollOver() throws IOException {
        ...
        if (maxBackupIndex > 0) {
            ...
            List<Long> maxBackupIndexDates = getMaxBackupIndexDates(folder);
            for(File ff : folder.listFiles()){
                if(!maxBackupIndexDates.contains(ff.lastModified())){
                    result = ff.delete();
                }
            }
...
//            }
    ...
    List<Long> getMaxBackupIndexDates(File folder){
        List<Long> result = new ArrayList<Long>();
        long[] b = new long[folder.listFiles().length];
        int i = 0;
        for(File file:folder.listFiles()){
            b[i] = file.lastModified();
            i++;
        }
        Arrays.sort(b);
        int t = b.length-1;
