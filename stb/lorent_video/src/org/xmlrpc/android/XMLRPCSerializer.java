...
class XMLRPCSerializer implements IXMLRPCSerializer {
	...
	public Object deserialize(XmlPullParser parser) throws XmlPullParserException, IOException {
		...
		try {
			...
			Log.i("deserialize:xml tag name", typeNodeName);
		...
		if (hasType) {
			...
			if (typeNodeName.equals(TYPE_STRUCT)) {
				...
				obj = map;
			}else if(typeNodeName.equals(TPYE_EX_SERIALIZABLE)){
				String value = parser.nextText();
				
				ObjectInputStream in = new ObjectInputStream(new ByteArrayInputStream(Base64Coder.decode(value)));
				obj = null;
				try {
					obj = in.readObject();
			...
			}else if(typeNodeName.equals(TYPE_EX_NIL)){
				obj = null;
				return obj;
			}  else {
