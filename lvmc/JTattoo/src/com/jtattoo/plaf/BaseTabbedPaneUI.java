...
// Private instance data
    ...
    class CreateTabButton extends JButton implements UIResource{

		public static final int WIDTH = 22;
		public static final int SPACE = 5;
    	
        public CreateTabButton() {
//        	this.setIcon(new ImageIcon(BaseTabbedPaneUI.class.getResource("icons/AddWB.png")));
//			this.setRolloverIcon(new ImageIcon(BaseTabbedPaneUI.class.getResource("icons/tab_addbtn_normal.png")) {
//
//				@Override
...
//							255, 255, 255, 200), 0, 10, new Color(255, 255,
//							255, 60));
//					g2.setPaint(paint);
//					g2.fillRect(x, y, 17, 17);
//				}
//			});
        	this.setIcon(new ImageIcon(BaseTabbedPaneUI.class.getResource("icons/tab_addbtn_normal.png")));
			...
			this.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if(tabPane instanceof CreatableTabPanel){
						((CreatableTabPanel) tabPane).createTab();
					}
				}
			});
		}
    ...
    protected Polygon createCroppedTabClip(int tabPlacement, Rectangle tabRect, int cropline) {
        int rlen = 0;
        int start = 0;
        int end = 0;
        int ostart = 0;
     ...
     */
    protected void paintCroppedTabEdge(Graphics g, int tabPlacement, int tabIndex, int x, int y) {
...
//            }
    ...
    private class TabbedPaneScrollLayout extends TabbedPaneLayout {
        ...
        private int getTotalTabWidth(int tc){
        	return rects[tc - 1].x + rects[tc - 1].width+CreateTabButton.WIDTH+CreateTabButton.SPACE;
        }
...
//                    ex.printStackTrace(outStream);
                ...
                for (int i = 0; i < numChildren; i++) {
                    ...
                    if (child instanceof ScrollableTabViewport) {
                        ...
                        switch (tabPlacement) {
                            ...
                            case TOP:
                            default:
                                int totalTabWidth = getTotalTabWidth(tc);
                                if (totalTabWidth > tw) {
                                    // Allow space for scrollbuttons
                                	...
                                	vw=totalTabWidth-CreateTabButton.WIDTH-CreateTabButton.SPACE/2;
                                }
                    ...
                    } else if(child instanceof CreateTabButton){
                    	Dimension size = child.getPreferredSize();
                    	int bx,by;
                    	if(tc!=0){
                    		int totalTabWidth = getTotalTabWidth(tc);
                    		if (totalTabWidth > tw) {
                    			bx=bounds.width - insets.left - space+CreateTabButton.SPACE;
                    		}else{
                    			bx = totalTabWidth-size.width;
                    		}
                    		by = ty + (th - size.height - tabAreaInsets.bottom) / 2;
                    		if (tabPlacement == BOTTOM) {
                    			by += tabAreaInsets.bottom;
                    		} else {
                    			by++;
                    		...
                    		bx=10;
                    		by=child.getBounds().y;
                    	}
                    	child.setBounds(bx, by==0?13:by, size.width, size.height);
                    }else {
    ...
    public class ScrollableTabSupport implements ChangeListener {
        ...
        public ScrollableTabViewport getViewport() {
			return viewport;
		}
