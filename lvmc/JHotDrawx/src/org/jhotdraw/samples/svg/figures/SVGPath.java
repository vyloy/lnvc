...
/**
 * SVGPath is a composite Figure which contains one or more BezierFigures as its
 * children.
 * <p>
 ...
 */
public class SVGPath extends AbstractAttributedCompositeFigure implements
		SVGFigure {
	...
	public void _drawFigure(Graphics2D g) {
		validatePath();
		...
		// g.drawString(this.toString()+" "+getChildCount(), (float)
		// getBounds().x, (float) getBounds().y);
	}

	public void drawFill(Graphics2D g) {
		if (getChildren().size() > 0 /*
									 * && ((BezierFigure)
									 * getChildren().get(0)).isClosed()
									 */) {
			g.fill(path);
		}
	}
	...
	protected void validatePath() {
		if (path == null) {
			...
			path.setWindingRule(WINDING_RULE.get(this) == WindingRule.EVEN_ODD ? GeneralPath.WIND_EVEN_ODD
	...
	protected void readPoints(DOMInput in) throws IOException {
		...
		if (in.getTagName().equals("polyline")) {
			...
			while (tt.hasMoreTokens()) {
				b.add(new BezierPath.Node(Double.valueOf(tt.nextToken()),
	...
	public Collection<Action> getActions(Point2D.Double p) {
		ResourceBundleUtil labels = ResourceBundleUtil
				.getLAFBundle("org.jhotdraw.samples.svg.Labels");
...
//		});
	...
	public JSONObject toJSON() {
		...
		json.put("d", buf.toString());
		SVGFigureUtil.addAttributesToJSONObject(this, json);
		return json;
	}
}
