    ...
    protected void endEdit() {
        if (typingTarget != null) {
            ...
            if (textField.getText().length() > 0) {
                ...
                if(typingTarget instanceof RemoteFigure){
                	Client.getInstance().broadcast(new ChangeAttribute((RemoteFigure) typingTarget,AttributeKeys.TEXT,textField.getText()));
                }
                if (createdFigure != null) {
                    getDrawing().fireUndoableEditHappened(creationEdit);
                    createdFigure = null;
