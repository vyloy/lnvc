...
public class BezierFigure extends AttributedFigure {
    ...
    protected void drawStroke(Graphics2D g) {
        if (isClosed()) {//曲线
            ...
            //System.out.println("g.draw(getCappedPath());");
        }
        drawCaps(g);
        //System.out.println("drawCaps(g);");
    }
    
    public void drawCaps(Graphics2D g) {
        if (getPointCount() > 1) {
     ...
     * Sets a control point.
     */
    public void remoteSetNode(int index, BezierPath.Node p) {
    	synchronized(queue){
    		if(!dragging)
    			path.set(index, p);
    	}
