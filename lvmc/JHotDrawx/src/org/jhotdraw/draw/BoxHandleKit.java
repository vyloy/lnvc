...
import org.jhotdraw.samples.svg.figures.ChangeFigureBounds;
...
public class BoxHandleKit {
    ...
    private static class ResizeHandle extends LocatorHandle {
        ...
        public void trackEnd(Point anchor, Point lead, int modifiersEx) {
			fireUndoableEditHappened(new GeometryEdit(getOwner(), geometry,
					getOwner().getRestoreData()));
			Figure f = getOwner();
			if (f instanceof RemoteFigure) {
				RemoteFigure rf = (RemoteFigure) f;
//				Client.getInstance().broadcast(new ChangeBounds(rf, f.getBounds()));
				Client.getInstance().broadcast(new ChangeFigureBounds(rf));
			}
        }
		...
		protected void setBounds(Point2D.Double anchor, Point2D.Double lead) {
			...
			Rectangle2D.Double newBounds = new Rectangle2D.Double(Math.min(
					anchor.x, lead.x), Math.min(anchor.y, lead.y),
					Math.abs(anchor.x - lead.x), Math.abs(anchor.y - lead.y));
