       ...
       // Check that our plugin is enabled.
        ...
        try {
            ...
            if ("add".equals(type)) {
                ...
                replyMessage("ok",response,out);
                //xmlProvider.sendInfo(request, response, presence);
            }else if("existGroup".equals(type)){
            	boolean ret = plugin.existGroup(groupNames);
            	replyMessage(ret + "",response,out);
            }else if("createShareGroup".equals(type)){
            	plugin.createShareGroup(groupNames);
            	replyMessage("ok",response,out);
            }else if("removeShareGroup".equals(type)){
            	plugin.removeShareGroup(groupNames);
            	replyMessage("ok",response,out);
            }else if("addGroupUser".equals(type)){
            	plugin.addGroupUser(groupNames, username);
            	replyMessage("ok",response,out);
            }else if("removeGroupUser".equals(type)){
            	plugin.removeGroupUser(groupNames, username);
            	replyMessage("ok",response,out);
            }else {
