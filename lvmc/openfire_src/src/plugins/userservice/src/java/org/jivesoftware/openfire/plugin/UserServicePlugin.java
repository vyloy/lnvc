...
public class UserServicePlugin implements Plugin, PropertyEventListener {
    ...
    public void createShareGroup(String groupName){
		try {
			Group group = GroupManager.getInstance().createGroup(groupName);
			group.getProperties().put("sharedRoster.displayName", groupName);
			group.getProperties().put("sharedRoster.groupList", "");
			group.getProperties().put("sharedRoster.showInRoster", "onlyGroup");
		} catch (GroupAlreadyExistsException e) {
			System.out.println("group " + groupName + " exist");
		}
    }
    
    public void removeShareGroup(String groupName){
		try {
    ...
    public void addGroupUser(String groupName, String username) throws Exception{
		try {
			Group group = GroupManager.getInstance().getGroup(groupName);
    		group.getMembers().add(server.createJID(username, null));
//    		Log.error("新增用户加入为好友", "googgogogo==================");
		} catch (GroupNotFoundException e) {
			System.out.println("group " + groupName + " not found");
