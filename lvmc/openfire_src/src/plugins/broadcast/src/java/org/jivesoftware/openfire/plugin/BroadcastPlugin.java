...
import org.jivesoftware.util.XMLUtil;
...
//        	String propertiesString = XMLUtil.getPropertiesString(message);
        	String[] usernameList = null;
        	try {
				Map<String, Object> properties = XMLUtil.getProperties(message);
				Object[] objectlist = (Object[]) properties.get("usernameList");
				if (objectlist != null) {
					usernameList = new String[objectlist.length];
					for (int i = 0; i < objectlist.length; i++) {
						usernameList[i] = (String) objectlist[i];
					}
				}
			} catch (Exception e) {
				Log.error(e.getMessage(), e);
			...
			if (usernameList != null) {
				//"@" + serviceName + "." + componentManager.getServerName()
				for (String username : usernameList) {
					Message newMessage = message.createCopy();
//					JID jid = new JID(username+"@" + serviceName + "." + componentManager.getServerName());
					String xmppdomain = "@" + JiveGlobals.getProperty("xmpp.domain");
