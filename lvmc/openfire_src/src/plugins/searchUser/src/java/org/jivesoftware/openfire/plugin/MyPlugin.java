...
import com.myplugin.test.TestObject;


//public class MyPlugin implements Component, Plugin, PropertyEventListener {
//	
//	private static final Logger Log = LoggerFactory.getLogger(SearchPlugin.class);
//	
//	public static final String NAMESPACE_JABBER_IQ_SEARCH = "jabber:iq:myplugin";
//    public static final String SERVICENAME = "myplugin.service";
...
//	
//    public MyPlugin() {
//        serviceName = JiveGlobals.getProperty(SERVICENAME, "searchUser");
...
class MyIQHandler extends IQHandler{   
    ...
    public MyIQHandler(){   
        super(moduleName);   
        info = new IQHandlerInfo("query", "jabber:iq:myplugin");//设置监听的命名空间   
    }   
...
//        System.out.println("myplugin:" + packet.toXML());
//        Element queryResult = DocumentHelper.createElement(QName.get("query",
//                "jabber:iq:myplugin"));
//        queryResult.addElement("instructions").addText("fff");
//        queryResult.addElement("first");
//        queryResult.addElement("last");
//        queryResult.addElement("nick");
//        queryResult.addElement("email");
//        reply.setChildElement(queryResult);
        //System.out.println("dddd" + packet.toXML());
        //reply.setTo("fqq@conference.localhost");
        Element query = packet.getChildElement();//取得客户端发送过来的xml 
        Element operate = query.element("operate");
        String toStr = operate.getText();
        System.out.println("toStr========" + toStr);
        try{
        	ByteArrayInputStream sis = new ByteArrayInputStream(StringUtils.decodeBase64(toStr));
        ...
        reply.setChildElement(query.createCopy());
//        reply.setTo(packet.getFrom());
//        
//        PacketExtension pe = new PacketExtension("query","myplugin:helloworld");
//        Element el = pe.getElement();
        
        //Element element = new Element("","");
        //Element el = reply.setChildElement("query","gggggg");
        //<item jid="nousergroup20111111042025" name="" subscription="none"><group>默认分组</group></item>
//        Element item = el.addElement("item");
//        item.addAttribute("id", "123456");
//        item.addAttribute("name", "");
//        item.addAttribute("subscription", "none");
//        Element group = item.addElement("group");
//        group.setText("aaaaa");
//        reply.addExtension(pe);
