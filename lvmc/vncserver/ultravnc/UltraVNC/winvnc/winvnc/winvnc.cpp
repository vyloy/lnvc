...
//	vnclog.SetLevel(10);
	...
	for (i = 0; i < strlen(szCmdLine); i++)
	{
		...
		if (strncmp(&szCmdLine[i], winvncRepeaterHost, strlen(winvncRepeaterHost)) == 0)
		{
			HWND hservwnd = NULL;
			...
			{
				COPYDATASTRUCT cpd;
				cpd.dwData = sizeof(COPYDATASTRUCT);
				cpd.cbData = strlen(szCmdLine)-i;
				cpd.lpData = (void*)(&szCmdLine[i]);
				SendMessage(hservwnd, WM_COPYDATA, (WPARAM)NULL,(LPARAM)&cpd);
			}
			return 0;
		}
