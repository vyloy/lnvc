	...
	case WM_COPYDATA:
		{
			...
			int i=strlen(winvncRepeaterHost);
			int start;
			char* pszId = NULL;
			start = i;
			while (strncmp( &DataBuf[start], "-connid", strlen("-connid") ) != 0) 
				start++;
			char * host=NULL;
			host = new char[start-i-1];
			strncpy(host,&DataBuf[i+1],start-i-2);
			host[start-i-2]=0;
			host=_strupr(host);
			start+=strlen("-connid");
			int end=start+1;
			//while(DataBuf[end]>' ')
			//	end++;
			while (strncmp( &DataBuf[end],";", strlen(";") ) != 0) 
				end++;
			char * idtmp=NULL;
			idtmp=new char[end-start];
			strncpy(idtmp,&DataBuf[start+1],end-start-1);
			idtmp[end-start-1]=0;
			idtmp=_strupr(idtmp);
			char id[20]={};
			strcat(id,"ID:");
			...
			tmpsock->Create();
			if (tmpsock->Connect(host, INCOMING_PORT_OFFSET))
			{
				_this->m_server->AddClient(tmpsock, TRUE, TRUE, 0, NULL, id, host, INCOMING_PORT_OFFSET);
			}
			delete[] DataBuf; 
			delete host;
			delete idtmp;
