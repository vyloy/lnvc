...
public class VncViewer extends java.applet.Applet implements
	...
	VncViewerListener vncViewerListener = null;
	...
	private Map<String, Object> customProperty = new HashMap<String, Object>();
	...
	/**
	 * @param vncViewerListener the vncViewerListener to set
	 */
	public void setVncViewerListener(VncViewerListener vncViewerListener) {
		this.vncViewerListener = vncViewerListener;
	}
	
	public void setViewOnly(boolean bflag){
		if (vc != null) {
			vc.enableInput(!bflag);
		}
	}
	...
	public void init() {
		...
		// authenticator = new AuthPanel(false); // mslogon support : go to
...
//			Panel canvasPanel = new Panel();
			ImageAwtPanel canvasPanel = new ImageAwtPanel();
			...
			vncViewerListener.createDesktopView(canvasPanel, buttonPanel);
...
//			vncViewerListener.createDesktopView(canvasPanel, buttonPanel);
	...
	void prologueDetectAuthProtocol() throws Exception {
		...
		rfb = new RfbProto(host, port, this, repeaterHost, repeaterPort); // Modif: troessner -
														// sf@2007: not yet used
	...
	boolean tryAuthenticate(String us, String pw) throws Exception {
		...
		case RfbProto.VncAuth:
			...
			if (mslogon) {
				...
				int n = JOptionPane.showConfirmDialog(vncFrame,
						"The current authentication method does not transfer your password securely."
			...
			if (mslogon) {
				// copy the us (user) parameter into the user Byte formated
				// variable
				...
				dm = ".";
				// copy the dm (domain) parameter into the domain Byte formated
				// variable
				...
				// copy the pw (password) parameter into the password Byte
				// formated variable
	...
	boolean AuthMsLogon(String us, String pw) throws Exception {
		...
		System.out.println("gen=" + gen + ", mod=" + mod + ", pub=" + pub
				+ ", key=" + key);
		...
		case RfbProto.VncAuthTooMany:
			throw new Exception(
					"MS-Logon (DH) authentication failed - too many tries");
		default:
			throw new Exception("Unknown MS-Logon (DH) authentication result "
					+ authResult);
		}
		return false;
	}
	...
	public void readParameters() {
		...
		str = readParameter("REPEATERHOST", false);
		if (str != null && !str.equals("")) {
			repeaterHost = str;
		}
		str = readParameter("REPEATERPORT", false);
		if (str != null && !str.equals("")) {
			repeaterPort = Integer.parseInt(str);
		}
...
//			System.exit(1);
		...
		vncViewerListener.onFatalError(str);
	}
